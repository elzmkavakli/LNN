...
Original file is located at
    https://colab.research.google.com/drive/1om7CZgwOi3u6EoylM7XuHMAcqrbmxCX3
"""
...
class LiquidNeuralNetwork:
    ...
    def train(self, x_train, y_train, x_val, y_val, epochs=50, learning_rate=0.01):
...
data['sin_t'] = np.sin(data['t'])
data['cos_t'] = np.cos(data['t'])

# Kullanılacak sütunları seç
features = ['t', 't_squared', 'sin_t', 'cos_t']
target = 'Close'  # Kapanış fiyatını hedef olarak alıyoruz

# Veriyi normalizasyon
X = data[features].values
y = (data[target] > data[target].median()).astype(int)  # İkili sınıflandırma
...
hidden_size = 200  # Gizli katman boyutunu artırdık
output_size = 2
...
num_images = 10
net.plot_predictions(x_test, y_test, y_pred_test, num_images)